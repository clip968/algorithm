    ...
    while(!s.empty()){
        ...
        if(current.move_idx < current.moves.size()){
            ...
            if(chk_knight(ny, nx, m, n)){
                ...
                vector<pair<int, int>> next_move_options;
                for(int i = 0; i < 8; i++){
                    int nnx = nx + direction[i].x;
                    int nny = ny + direction[i].y;
                    if(chk_knight(nny, nnx, m, n)){
                        int deg = warnsdorff(nny, nnx, m, n);
                        next_move_options.push_back({deg, i});
                    }
                }
                sort(next_move_options.begin(), next_move_options.end());
                vector<int> next_moves;
                int len = next_move_options.size();
                for(int i=0;i < min(2, len);i++){
                    next_moves.push_back(next_move_options[i].second);
                }
